    ...
    def get_valid_moves(self, piece):
        ...
        for dx, dy, max_dist in piece.get_move_directions():
            for step in range(1, max_dist + 1 if max_dist != float("inf") else MAX_LINES):
                ...
                if has_val_at_key(self.pieces, (target_x, target_y)):
    ...
    def get_valid_placement_squares(self, player):
        ...
        for piece in self.pieces.values():
            if piece.player == player:
                if piece.piece_type == "palace":
                    self.get_palace_placement_squares(valid_placement_squares, piece)
                else:
                    valid_placement_squares.extend(self.get_valid_moves(piece))

        return valid_placement_squares

    def get_palace_placement_squares(self, valid_placement_squares, piece):
        palace_pos = (piece.cx, piece.cy)
        for dx in range(-2, 3):
            for dy in range(-2, 3):
                target_x = palace_pos[0] + dx
                target_y = palace_pos[1] + dy
                if 0 <= target_x < MAX_LINES - 1 and 0 <= target_y < MAX_LINES - 1:
                    if (target_x, target_y) not in self.pieces:
                        valid_placement_squares.append((target_x, target_y))
        return valid_placement_squares
        ...
        if self.selected_piece_on_board is not None:
            ...
            for target_x, target_y in valid_moves:
                ...
                if is_capture:
                    ...
                    pygame.draw.circle(surface, (200, 200, 100, 180),
                                       (int(self.cell_size / 2), int(self.cell_size / 2)),
